...
def _():
    ...
    import os
    import math
    from sklearn.preprocessing import RobustScaler # <-- Importar
...
@app.cell(disabled=True)
def _(df, df_feat, pd):
    # 1. Calcular estadísticas por producto
    stats_producto = df.groupby('product_id')['quantity_available'].agg(
        stock_promedio='mean',
        stock_volatilidad='std'
    ).reset_index()

    # Rellenar volatilidad 0 para productos con 1 solo registro
    stats_producto['stock_volatilidad'] = stats_producto['stock_volatilidad'].fillna(0)

    # 2. Crear los segmentos (Ejemplo simple con la mediana)
    #    Vamos a dividir por la mitad: productos de stock "Bajo" y "Alto"
    mediana_stock = stats_producto['stock_promedio'].median()

    print(f"La mediana del stock promedio es: {mediana_stock}")

    # 3. Etiquetar cada producto
    stats_producto['segmento'] = stats_producto['stock_promedio'].apply(
        lambda x: 'Stock_Alto' if x >= mediana_stock else 'Stock_Bajo'
    )

    # 4. Unir esta etiqueta a tu DataFrame principal
    #    Necesitamos un diccionario para mapear rápido
    mapa_segmentos = pd.Series(stats_producto.segmento.values, index=stats_producto.product_id).to_dict()

    # df_feat es el DataFrame que usas en la Fase 1
    # Asumimos que df_feat se crea a partir de df_original
    df_feat['segmento'] = df_feat['product_id'].map(mapa_segmentos)

    print("\nSegmentación completada:")
    print(stats_producto['segmento'].value_counts())
    return
...
def _(LabelEncoder, df_feat, joblib, pd):
    ...
    categorias_onehot = ['warehouse_location', 'stock_status', 'segmento']
    df_proc = pd.get_dummies(df_proc, columns=categorias_onehot, drop_first=True)

    print("\nColumnas después de One-Hot Encoding (CON SEGMENTO):")
    # Vamos a verificar el nuevo nombre de la columna
    nuevas_columnas = [col for col in df_proc.columns if 'segmento_' in col]
    print(f"Nuevas columnas de segmento: {nuevas_columnas}")
    return (df_proc,)


@app.cell(disabled=True)
def _(RobustScaler, df_proc, joblib):
    columnas_numericas = [
    ...
    # 1. CAMBIO DE ESCALADOR
    # scaler = MinMaxScaler() # <-- Comentado
    scaler = RobustScaler() # <-- Nuevo
...
@app.cell(disabled=True)
def _(df_proc, pd):
    df_proc_path = 'df_processed_features_mejorado.csv'
...
@app.cell(disabled=True)
def _(Adam, EarlyStopping, ModelCheckpoint, model, tf):
    # --- Compilar el modelo 
    ...
    model.compile(
        ...
        loss=tf.keras.losses.Huber(),  # <-- ESTE ES EL CAMBIO
...
def _(history, plt):
    ...
    # Gráfico de Pérdida (Loss - Huber)
    plt.subplot(1, 2, 1)
    plt.plot(epochs_range, loss, label='Pérdida de Entrenamiento (Huber)')
    plt.plot(epochs_range, val_loss, label='Pérdida de Validación (Huber)')
...
def _(
    ...
    # Des escalar
    scaler_1 = joblib.load('robust_scaler.joblib')
    #scaler = joblib.load('min_max_scaler.joblib')
    TARGET_COLUMN_INDEX = 2
    num_numeric_features = 18  # <--- ¡CAMBIO HECHO!
    ...
    min_stock = y_val_real.min()
    max_stock = y_val_real.max()
    ...
    # Ya no usamos el scaler, usamos los datos reales que des-escalamos.
    print(f'   • RMSE: {rmse_scaled:.4f}')
    print(f'   • MAE:  {mae_scaled:.4f}')
...
@app.cell(disabled=True)
def _(FEATURE_COLUMNS, joblib, load_model, pd):
    N_STEPS_1 = 7
    ...
    try:
        scaler_2 = joblib.load('robust_scaler.joblib')
    # escalador
        print("Escalador 'robust_scaler.joblib' cargado.")
    except Exception as e:
        print(f"Error al cargar 'robust_scaler.joblib': {e}")
    try:
    ...
    try:
        df_features = pd.read_csv('df_processed_features_mejorado.csv')
    ...
    class ModeloStockEvaluador:
      ...
      • Error relativo: 2.99 del experimento
        ...
        def preprocesar_datos(self):
            ...
            # ##### CAMBIO 1: Añadir SEGMENTACIÓN aquí #####
            print("   • Segmentando productos...")
            stats_producto = df.groupby('product_id')['quantity_available'].agg(
                stock_promedio='mean'
            ).reset_index()
            mediana_stock = stats_producto['stock_promedio'].median()
            ...
            mapa_segmentos = pd.Series(stats_producto.segmento.values, index=stats_producto.product_id).to_dict()
            df['segmento'] = df['product_id'].map(mapa_segmentos)
      ...
      # One-Hot Encoding
        ...
        def entrenar_modelo(self, epochs=100, batch_size=64, verbose=0):
            ...
            self.model.compile(optimizer=Adam(learning_rate=0.001), loss=tf.keras.losses.Huber(), metrics=['mean_absolute_error'])
    ...
    class EvaluadorMultiDataset:
        ...
        def generar_reporte_completo(self, guardar=True):
            ...
            if guardar:
                ...
                output_path = Path('resultados_comparacion_mejorado.csv')
...
def _(evaluador_multi, np):
    ...
    DATASET_ESPECIFICO = 'subset_500_productos'  # Cambia esto al dataset que quieras analizar
